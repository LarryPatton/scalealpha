    ...
    <div class="fixed right-8 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-4">
      ...
      >
        <span 
          ...
          :class="isDark ? 'bg-gray-800 text-white' : 'bg-gray-900 text-white shadow-lg'"
        >
          {{ section.label }}
        </span>
        <div 
          ...
          :class="activeSection === section.id 
            ? (isDark ? 'bg-sky-600 border-sky-600 scale-125' : 'bg-sky-600 border-sky-600 scale-125 shadow-lg') 
            : (isDark ? 'bg-transparent border-gray-600 hover:border-gray-400' : 'bg-transparent border-gray-400 hover:border-gray-600')"
        ></div>
      </button>
    </div>
    ...
    >
      ...
      <div class="max-w-[1800px] mx-auto px-6 lg:px-12 xl:px-24 grid lg:grid-cols-12 gap-12 lg:gap-24 items-center relative z-10 w-full">
        ...
        <div class="lg:col-span-5 flex flex-col justify-center text-left reveal-on-scroll reveal-left">
          ...
          <h1 class="text-5xl md:text-7xl font-bold mb-6 leading-tight tracking-tight" :style="{ color: isDark ? tokens.colors.text.primary : '#111827' }">
            ScaleAlpha.ai
          </h1>
          <p class="text-xl md:text-2xl font-semibold mb-6" :style="{ color: isDark ? tokens.colors.accent.primary : '#0284c7' }">
            个性化AI对冲基金团队
          </p>
          <p class="text-lg mb-10 leading-relaxed max-w-lg" :style="{ color: isDark ? tokens.colors.text.tertiary : '#4b5563' }">
    ...
    <div class="w-full">
      ...
      <section id="themes" class="w-full py-24 lg:py-32 flex items-center min-h-[80vh] bg-transparent">
        <div class="max-w-[1800px] mx-auto px-6 lg:px-12 xl:px-24 grid lg:grid-cols-12 gap-12 lg:gap-24 items-center w-full">
          ...
          <div class="order-1 lg:order-2 lg:col-span-5 flex flex-col justify-center reveal-on-scroll reveal-right delay-200">
            <h2 class="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight" :style="{ color: isDark ? tokens.colors.text.primary : '#111827' }">主题发现</h2>
            <p class="text-lg md:text-xl mb-6" :style="{ color: isDark ? tokens.colors.text.tertiary : '#374151' }">追踪热点主题，把握投资周期</p>
            <p class="text-base leading-relaxed mb-8" :style="{ color: isDark ? tokens.colors.text.muted : '#4b5563' }">实时监控市场热门主题的生命周期演变，从萌芽期到成熟期，帮助您在最佳时机入场或离场。每个主题都配有30天/1年涨跌幅数据和相关标的列表。</p>
            <ul class="space-y-3 mb-8">
              <li class="flex items-center gap-3 text-base" :style="{ color: isDark ? tokens.colors.text.secondary : '#374151' }">
      ...
      <section id="opportunities" class="w-full py-24 lg:py-32 flex items-center min-h-[80vh] bg-transparent">
        <div class="max-w-[1800px] mx-auto px-6 lg:px-12 xl:px-24 grid lg:grid-cols-12 gap-12 lg:gap-24 items-center w-full">
          ...
          <div class="lg:col-span-5 flex flex-col justify-center reveal-on-scroll reveal-left">
            <h2 class="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight" :style="{ color: isDark ? tokens.colors.text.primary : '#111827' }">机会浏览</h2>
            <p class="text-lg md:text-xl mb-6" :style="{ color: isDark ? tokens.colors.text.tertiary : '#374151' }">AI每日精选高置信度交易机会</p>
            <p class="text-base leading-relaxed mb-8" :style="{ color: isDark ? tokens.colors.text.muted : '#4b5563' }">基于多维度分析的智能评级系统，每个机会都经过风险收益比计算、时间框架预估和风险等级评定。支持推荐和关注双模式，让您高效筛选目标。</p>
            <ul class="space-y-3 mb-8">
              <li class="flex items-center gap-3 text-base" :style="{ color: isDark ? tokens.colors.text.secondary : '#374151' }">
      ...
      <section id="attribution" class="w-full py-24 lg:py-32 flex items-center min-h-[80vh] bg-transparent">
        <div class="max-w-[1800px] mx-auto px-6 lg:px-12 xl:px-24 grid lg:grid-cols-12 gap-12 lg:gap-24 items-center w-full">
          ...
          <!-- Text Area - Right -->
          <div class="order-1 lg:order-2 lg:col-span-5 flex flex-col justify-center reveal-on-scroll reveal-right delay-200">
            <h2 class="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight" :style="{ color: isDark ? tokens.colors.text.primary : '#111827' }">事件归因</h2>
            <p class="text-lg md:text-xl mb-6" :style="{ color: isDark ? tokens.colors.text.tertiary : '#374151' }">理解股价波动背后的真正原因</p>
            <p class="text-base leading-relaxed mb-8" :style="{ color: isDark ? tokens.colors.text.muted : '#4b5563' }">通过时间线可视化展示影响股价的重大事件，每个事件都标注好评/利空/中性情绪标签。支持按股票代码筛选，关联分析多标的受影响程度。</p>
            <ul class="space-y-3 mb-8">
              <li class="flex items-center gap-3 text-base" :style="{ color: isDark ? tokens.colors.text.secondary : '#374151' }">
      ...
      <section id="strategy" class="w-full py-24 lg:py-32 flex items-center min-h-[80vh] bg-transparent">
        <div class="max-w-[1800px] mx-auto px-6 lg:px-12 xl:px-24 grid lg:grid-cols-12 gap-12 lg:gap-24 items-center w-full">
          <!-- Text Area - Left -->
          <div class="lg:col-span-5 flex flex-col justify-center reveal-on-scroll reveal-left">
            <h2 class="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight" :style="{ color: isDark ? tokens.colors.text.primary : '#111827' }">策略生成</h2>
            <p class="text-lg md:text-xl mb-6" :style="{ color: isDark ? tokens.colors.text.tertiary : '#374151' }">定制专属于您的AI交易策略</p>
            <p class="text-base leading-relaxed mb-8" :style="{ color: isDark ? tokens.colors.text.muted : '#4b5563' }">选择标的、配置分析框架、设定投资周期和风险偏好，还可输入您的个人市场洞察。AI将综合所有参数，生成专属于您的交易策略报告。</p>
            <ul class="space-y-3 mb-8">
              <li class="flex items-center gap-3 text-base" :style="{ color: isDark ? tokens.colors.text.secondary : '#374151' }">
      ...
      <section id="management" class="w-full py-24 lg:py-32 flex items-center min-h-[80vh] bg-transparent">
        <div class="max-w-[1800px] mx-auto px-6 lg:px-12 xl:px-24 grid lg:grid-cols-12 gap-12 lg:gap-24 items-center w-full">
          ...
          <!-- Text Area - Right -->
          <div class="order-1 lg:order-2 lg:col-span-5 flex flex-col justify-center reveal-on-scroll reveal-right delay-200">
            <h2 class="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight" :style="{ color: isDark ? tokens.colors.text.primary : '#111827' }">策略管理</h2>
            <p class="text-lg md:text-xl mb-6" :style="{ color: isDark ? tokens.colors.text.tertiary : '#374151' }">高效管理您的策略资产库</p>
            <p class="text-base leading-relaxed mb-8" :style="{ color: isDark ? tokens.colors.text.muted : '#4b5563' }">实时追踪策略生成进度，智能筛选高评级或待更新策略。支持自定义分组收藏、批量操作，让您的策略库井然有序，随时调取最优方案。</p>
            <ul class="space-y-3 mb-8">
              <li class="flex items-center gap-3 text-base" :style="{ color: isDark ? tokens.colors.text.secondary : '#374151' }">
...
const { setSection } = useScrollState()
