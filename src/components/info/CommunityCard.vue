<template>
  <div class="bg-[#2a2a2a] rounded-xl p-5 border border-[#404040] hover:border-green-500 transition cursor-pointer group h-full flex flex-col">
    <div class="flex items-center gap-3 mb-4">
      <div class="w-8 h-8 rounded-full bg-gradient-to-br from-green-400 to-teal-500 flex items-center justify-center text-white text-xs font-bold">
        {{ author.charAt(0) }}
      </div>
      <div>
        <h3 class="text-sm font-bold text-white">{{ author }}</h3>
        <p class="text-[10px] text-gray-500">{{ time }}</p>
      </div>
    </div>
    
    <div class="flex-grow">
      <h4 class="font-bold text-white mb-2 group-hover:text-green-400 transition">{{ title }}</h4>
      <p class="text-sm text-gray-300 line-clamp-3 mb-3">{{ content }}</p>
    </div>

    <div class="flex items-center justify-between mt-auto pt-3 border-t border-[#404040]">
      <div class="flex gap-2">
        <span class="px-2 py-1 bg-green-500/10 rounded text-[10px] text-green-400 border border-green-500/20">Community</span>
      </div>
      <div class="flex items-center gap-3 text-gray-500 text-xs">
        <span class="flex items-center gap-1">
          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
          {{ likes }}
        </span>
        <span class="flex items-center gap-1">
          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path></svg>
          {{ comments }}
        </span>
      </div>
    </div>
  </div>
</template>

<script setup>
defineProps({
  author: String,
  time: String,
  title: String,
  content: String,
  likes: Number,
  comments: Number
})
</script>
