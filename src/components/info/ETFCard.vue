<template>
  <div class="bg-[#2a2a2a] rounded-xl p-5 border border-[#404040] hover:border-purple-500 transition cursor-pointer group h-full flex flex-col relative">
    <!-- Add to Watchlist Button -->
    <button 
      @click.stop="toggleWatchlistMenu"
      class="absolute top-3 right-3 p-1.5 rounded-lg bg-[#1a1a1a] border border-[#404040] hover:border-purple-500 opacity-0 group-hover:opacity-100 transition-all z-10"
      :title="isFollowing ? '管理自选组' : '添加到自选组'"
    >
      <svg 
        :class="[
          'w-4 h-4 transition',
          isFollowing ? 'text-purple-400' : 'text-gray-400'
        ]"
    ...
    <div class="flex justify-between items-start mb-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h3 class="font-bold text-white group-hover:text-purple-400 transition">{{ code }}</h3>
    ...
    <div class="flex flex-wrap gap-2 mt-auto pt-3 border-t border-[#404040]">
      <span class="px-2 py-1 bg-purple-500/10 rounded text-[10px] text-purple-400 border border-purple-500/20">ETF</span>
