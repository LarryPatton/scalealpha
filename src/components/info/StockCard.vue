<template>
  <div class="bg-[#2a2a2a] rounded-xl p-5 border border-[#404040] hover:border-orange-500 transition cursor-pointer group h-full flex flex-col relative">
    <!-- Add to Watchlist Button -->
    <button 
      @click.stop="toggleWatchlistMenu"
      class="absolute top-3 right-3 p-1.5 rounded-lg bg-[#1a1a1a] border border-[#404040] hover:border-orange-500 opacity-0 group-hover:opacity-100 transition-all z-10"
      :title="isFollowing ? '管理自选组' : '添加到自选组'"
    >
      <svg 
        :class="[
          'w-4 h-4 transition',
          isFollowing ? 'text-orange-400' : 'text-gray-400'
        ]"
    ...
    <div class="flex justify-between items-start mb-4">
      <div class="flex items-center gap-3">
        <div class="w-10 h-10 rounded-lg bg-orange-500/20 flex items-center justify-center text-orange-400 font-bold">
          {{ code.charAt(0) }}
        </div>
        <div>
          <h3 class="font-bold text-white group-hover:text-orange-400 transition">{{ code }}</h3>
          <p class="text-xs text-gray-400">{{ name }}</p>
        </div>
      </div>
      <div class="text-right mr-8">
        <div class="text-lg font-bold text-white">${{ price }}</div>
        <div :class="change >= 0 ? 'text-red-400' : 'text-green-400'" class="text-xs font-bold">
          {{ change >= 0 ? '+' : '' }}{{ change }}%
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-wrap gap-2 mt-auto pt-3 border-t border-[#404040]">
      <span class="px-2 py-1 bg-orange-500/10 rounded text-[10px] text-orange-400 border border-orange-500/20">Stock</span>
